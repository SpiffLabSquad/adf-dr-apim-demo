...
<#
.SYNOPSIS
  Flips the APIM 'active-region' named value so the /adf/trigger endpoint routes to a
...
$apim = az deployment group show -g $ResourceGroup -n main --query properties.outputs.apimName.value -o tsv
...
Write-Host "Done. https://$apim.azure-api.net/adf/trigger/<pipeline> now routes to the $Region factory (no app change)."
